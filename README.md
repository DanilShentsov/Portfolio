<!-- Profile Header -->
<p align="center">
  <img src="https://raw.githubusercontent.com/DanilShentsov/Portfolio/main/banner.png" alt="Banner" width="80%"/>
</p>

<h1 align="center">Привет, я Данил – местами программист <img src="https://raw.githubusercontent.com/ABSphreak/ABSphreak/master/gifs/Hi.gif" width="30px"/></h1>

<p align="center">
  <em>Аспирант • Аналитик больших данных • Веб-разработчик </em>
</p>

---

## 🚀 Обо мне

- 🎓 **Аспирант ИТМО** по направлению 1.2.1. "Искусственный интеллект и машинное обучение"  
- 💼 **Аналитик больших данных** (Spark + Hadoop + Python + Clickhouse + Superset) в государственном секторе
- 🖥️ **Fullstack‑разработка на Django + React** — в качестве увлечения и саморазвития

---

## 🧰 Стек технологий

![Python](https://img.shields.io/badge/-Python-3776AB?style=flat&logo=python&logoColor=fff)
![Pandas](https://img.shields.io/badge/-Pandas-150458?style=flat&logo=pandas)
![Apache Spark](https://img.shields.io/badge/-Apache%20Spark-E25A1C?style=flat&logo=apache-spark&logoColor=fff)
![Hadoop](https://img.shields.io/badge/-Hadoop-66CCFF?style=flat&logo=apache-hadoop&logoColor=000)
![SQL](https://img.shields.io/badge/-SQL-4479A1?style=flat&logo=mysql&logoColor=fff)
![ClickHouse](https://img.shields.io/badge/-ClickHouse-ffdc00?style=flat)
![Apache Superset](https://img.shields.io/badge/-Apache%20Superset-1F66AB?style=flat&logo=apache)
![Django](https://img.shields.io/badge/-Django-092E20?style=flat&logo=django)
![React](https://img.shields.io/badge/-React-61DAFB?style=flat&logo=react&logoColor=000)
![TailwindCSS](https://img.shields.io/badge/-TailwindCSS-38B2AC?style=flat&logo=tailwind-css&logoColor=fff)
![Docker](https://img.shields.io/badge/-Docker-0db7ed?style=flat&logo=docker&logoColor=fff)
![Git](https://img.shields.io/badge/-Git-F05032?style=flat&logo=git&logoColor=fff)
![Nginx](https://img.shields.io/badge/-Nginx-009639?style=flat&logo=nginx&logoColor=white)

---

## 📂 Проекты

### 1. DS‑проекты *(репозиторий* **[Portfolio](https://github.com/DanilShentsov/Portfolio)** *)*

- **Airports Passenger Flow** — EDA и прогнозирование пассажиропотока аэропортов  
- **Diamonds ML** — прогноз цены бриллиантов  
- **Income ML** — классификация уровня дохода (формат Kaggle)  
- **Two MA Strategy** — бэктест торговой стратегии на двух скользящих средних  

> Полный набор ноутбуков, слайдов и учебных материалов — в репозитории **Portfolio**.

### 2. Веб‑проекты

#### **Сайт по заказу индивидуальных экскурсий** — [ivangid.ru](http://ivangid.ru)
**Стек:** Django · DRF · React · Tailwind · Gunicorn · Nginx

Одностраничное приложение для бронирования индивидуальных экскурсий по Абхазии, Сочи и Югу России.

| Ключевые функции | Подробнее |
|------------------|-----------|
| Загрузка изображений направлений через Django‑админку | `ImageField` + отображение на фронте |
| Умный поиск с автокомплитом | Задержка 300 мс, срабатывание от 3 символов |
| Группировка по регионам | Абхазия — зелёный бейдж, Сочи — бирюзовый, Юг России — синий |
| Форма бронирования | Маршрут, имя, телефон → письмо‑уведомление через SMTP Yandex |
| Всплывающее окно «Подробнее» | Вместо перехода на отдельную страницу |
| Блок отзывов | Удобная форма обратной связи - карточки отзывов |
| Развёртывание | Gunicorn + Nginx, домен ivangid.ru |

#### **Персональная страница** — [danilshentsov.github.io](https://danilshentsov.github.io)
Статическая визитка на GitHub Pages + мини‑игра.

---

## 🏆 Хакатоны

| Год | Мероприятие | Продукт | Роль | Итог |
|-----|-------------|-------------|-------------|-------------|
| 2023 | **Цифровой прорыв 2023: Сезон ИИ (Международный)** | Прогнозирование коммунальных издержек фонда недвижимости | Подготовка презентации, выступление | 7 место |
| 2023 | **Check AI Hack - Антиплагиат** | Обнаружение сгенерированного текста | Подготовка презентации, выступление | 2 место |
| 2023 | **Арк-Тек: 2023** | Сервис по оптимальному размещению объектов городской инфраструктуры | Подготовка презентации, выступление | Финал / Топ-10 |
| 2023 | **Лидеры цифровой трансформации 2023: Краснодар** | Детекция нарушений в сфере нелегальной нестационарной торговли по съемкам с видеокамеры | Подготовка презентации, выступление | 4 место |
| 2023 | **Цифровой прорыв 2023: Сезон ИИ (ЮФО)** | ML для оценки параметров эконометрической модели | Подготовка презентации, выступление | 3 место |
| 2023 | **Цифровой прорыв 2023: Сезон ИИ (ЦФО)** | Агрегирование данных по лекарственным травам РФ из открытых источников | Подготовка презентации, выступление, аналитика данных | 5 место |
| 2023 | **Лидеры цифровой трансформации 2023** | Система поиска нарушений в сфере захоронений отходов | Подготовка презентации, выступление | Топ-10 |
| 2023 | **Машинное обучение: no-code кластеризация текста (Финтех Хаб ЦБ)** | Разработка цифрового продукта | Прототипирование, отладка фронтэнда | Финалист |
| 2022 | **Сбер акселератор** | Цифровой сурдопереводчик | Лидер стартапа | Финалист, Топ-20 из 1300 проектов, региональная  |

## 🎤 Конференции и доклады

| Дата | Тема | Площадка |
|------|------|----------|
| 04.2025 | Универсальный DataOps-конвейер для работы с данными в государственном секторе | Конгресс молодых учёных ИТМО |
| 01.2025 | Эффективность табличных соединений в Hadoop при работе с большими данными | LIV научная и учебно-методическая конференция университета ИТМО |
| 04.2024 | Оценка качества больших данных в государственном секторе | Конгресс молодых учёных ИТМО |
| 01.2024 | Большие данные в государственном и муниципальном управлении: актуальные тренды и перспективы | LIII научная и учебно-методическая конференция университета ИТМО |

---

## 📈 GitHub-статистика

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=DanilShentsov&show_icons=true&theme=default" alt="stats"/>
</p>

---

## 📬 Связаться со мной

[![Telegram](https://img.shields.io/badge/Telegram-26A5E4?style=flat&logo=telegram&logoColor=white)](https://t.me/ly_gunn) 

---
